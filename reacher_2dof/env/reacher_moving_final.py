...
class ReacherMovingEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, boxes):
        ...
        if boxes:
            print("boxes")
            mujoco_env.MujocoEnv.__init__(self, cwd+'/env/reacher_box.xml', 2)
        else:
            print("no_boxes")
            mujoco_env.MujocoEnv.__init__(self, cwd+'/env/reacher_no_box.xml', 2)
    ...
    def reset_model(self):
        ...
        if self.boxes:
            box_poses = self.np_random.uniform(low=-1.5, high=1.5, size=4)

            self.model.body_pos[5][0] = box_poses[0]
            self.model.body_pos[5][1] = box_poses[1]

            self.model.body_pos[6][0] = box_poses[2]
            self.model.body_pos[6][1] = box_poses[3]

            #qpos[2:4] = box_poses[0:2]
            #qpos[9:11] = box_poses[2:4]
